...
constexpr void test_no_implicit_conversion() {
  ...
  PREVIEW_STATIC_ASSERT(std::is_constructible<ll_mapping_t<int, D>, lr_mapping_t<int, 5>>::value);
  PREVIEW_STATIC_ASSERT(std::is_convertible<lr_mapping_t<int, 5>, ll_mapping_t<int, D>>::value);

  // Check that dynamic to static conversion only works explicitly
  PREVIEW_STATIC_ASSERT(std::is_constructible<ll_mapping_t<int, 5>, lr_mapping_t<int, D>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_convertible<lr_mapping_t<int, D>, ll_mapping_t<int, 5>>::value);

  // Sanity check that smaller index_type to larger index_type conversion works
  PREVIEW_STATIC_ASSERT(std::is_constructible<ll_mapping_t<size_t, 5>, lr_mapping_t<int, 5>>::value);
  PREVIEW_STATIC_ASSERT(std::is_convertible<lr_mapping_t<int, 5>, ll_mapping_t<size_t, 5>>::value);

  // Check that larger index_type to smaller index_type conversion works explicitly only
  PREVIEW_STATIC_ASSERT(std::is_constructible<ll_mapping_t<int, 5>, lr_mapping_t<size_t, 5>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_convertible<lr_mapping_t<size_t, 5>, ll_mapping_t<int, 5>>::value);
}

constexpr void test_rank_mismatch() {
  constexpr size_t D = preview::dynamic_extent;

  PREVIEW_STATIC_ASSERT(!std::is_constructible<lr_mapping_t<int, D>, lr_mapping_t<int>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_constructible<lr_mapping_t<int>, lr_mapping_t<int, D, D>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_constructible<lr_mapping_t<int, D>, lr_mapping_t<int, D, D>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_constructible<lr_mapping_t<int, D, D, D>, lr_mapping_t<int, D, D>>::value);
}

constexpr void test_static_extent_mismatch() {
  PREVIEW_STATIC_ASSERT(!std::is_constructible<ll_mapping_t<int, 5>, lr_mapping_t<int, 4>>::value);
}

constexpr void test_rank_greater_one() {
  constexpr size_t D = preview::dynamic_extent;

  PREVIEW_STATIC_ASSERT(!std::is_constructible<ll_mapping_t<int, D, D>, lr_mapping_t<int, D, D>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_constructible<ll_mapping_t<int, 1, 1>, lr_mapping_t<int, 1, 1>>::value);
  PREVIEW_STATIC_ASSERT(!std::is_constructible<ll_mapping_t<int, D, D, D>, lr_mapping_t<int, D, D, D>>::value);
}
