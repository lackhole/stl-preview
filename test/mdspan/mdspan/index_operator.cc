...
constexpr void test_iteration(Mapping m) {
  ...
  using MDS = preview::mdspan<int, typename Mapping::extents_type, typename Mapping::layout_type>;
  MDS mds(data.data(), m);
...
constexpr void test_layout() {
  ...
  test_iteration(construct_mapping(Layout(), preview::extents<int>()));
  test_iteration(construct_mapping(Layout(), preview::extents<unsigned, D>(1)));
  test_iteration(construct_mapping(Layout(), preview::extents<unsigned, D>(7)));
  test_iteration(construct_mapping(Layout(), preview::extents<unsigned, 7>()));
  test_iteration(construct_mapping(Layout(), preview::extents<unsigned, 7, 8>()));
  test_iteration(construct_mapping(Layout(), preview::extents<signed char, D, D, D, D>(1, 1, 1, 1)));

// TODO(LLVM 20): Enable this once AppleClang is upgraded
#ifndef TEST_COMPILER_APPLE_CLANG
  int data[1];
  // Check operator constraint for number of arguments
  static_assert(check_operator_constraints(preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), 0));
  static_assert(
      !check_operator_constraints(preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), 0, 0));

  // Check operator constraint for convertibility of arguments to index_type
  static_assert(
      check_operator_constraints(preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), IntType(0)));
  static_assert(!check_operator_constraints(
      preview::mdspan(data, construct_mapping(Layout(), preview::extents<unsigned, D>(1))), IntType(0)));

  // Check operator constraint for no-throw-constructibility of index_type from arguments
  static_assert(!check_operator_constraints(
      preview::mdspan(data, construct_mapping(Layout(), preview::extents<unsigned char, D>(1))), IntType(0)));
  ...
  static_assert(check_operator_constraints(
      preview::mdspan(data, construct_mapping(Layout(), preview::extents<unsigned char, D, D>(1, 1))), int(0), size_t(0)));
  static_assert(check_operator_constraints(
      preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D, D>(1, 1))), unsigned(0), IntType(0)));

  constexpr bool t = true;
  constexpr bool o = false;
  static_assert(!check_operator_constraints(
      preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D, D>(1, 1))),
      unsigned(0),
      IntConfig<o, o, t, t>(0)));
  static_assert(check_operator_constraints(
      preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D, D>(1, 1))),
  ...
  if constexpr (std::is_same_v<Layout, preview::layout_left>) {
    static_assert(!check_operator_constraints(
        preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), std::array{IntConfig<o, o, t, t>(0)}));
    static_assert(!check_operator_constraints(
        preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), std::array{IntConfig<o, t, t, t>(0)}));
    static_assert(!check_operator_constraints(
        preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), std::array{IntConfig<t, o, o, t>(0)}));
    static_assert(!check_operator_constraints(
        preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), std::array{IntConfig<t, t, o, t>(0)}));
    static_assert(check_operator_constraints(
        preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), std::array{IntConfig<t, o, t, o>(0)}));
    static_assert(check_operator_constraints(
        preview::mdspan(data, construct_mapping(Layout(), preview::extents<int, D>(1))), std::array{IntConfig<t, t, t, t>(0)}));
...
constexpr void test_layout_large() {
  constexpr size_t D = preview::dynamic_extent;
  test_iteration(construct_mapping(Layout(), preview::extents<int64_t, D, 4, D, D>(3, 5, 6)));
  test_iteration(construct_mapping(Layout(), preview::extents<int64_t, D, 4, 1, D>(3, 6)));
}
...
constexpr bool test_large() {
  test_layout_large<preview::layout_left>();
  test_layout_large<preview::layout_right>();
  return true;
}
