...
constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc) {
  ...
  MDS m(m_org);
  PREVIEW_STATIC_ASSERT(noexcept(MDS(m_org)) == (noexcept(H(handle))&& noexcept(M(map))&& noexcept(A(acc))));
...
constexpr void mixin_layout(const H& handle, const A& acc) {
  ...
  PREVIEW_STATIC_ASSERT(std::is_trivially_copyable<preview::layout_left::mapping<preview::extents<int>>>::value);
  ...
  PREVIEW_STATIC_ASSERT(!std::is_trivially_copyable<layout_wrapping_integral<4>::mapping<preview::extents<int>>>::value);
