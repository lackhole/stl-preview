...
constexpr void mixin_layout(const H& handle, const A& acc) {
  ...
  static_assert(std::is_constructible_v<layout_wrapping_integral<8>::mapping<preview::extents<int>>, preview::extents<int>>);
  static_assert(
      !std::is_constructible_v<layout_wrapping_integral<8>::mapping<preview::extents<int>>, const preview::extents<int>&>);
  ...
  static_assert(!std::is_constructible_v<layout_wrapping_integral<4>::mapping<preview::extents<int>>, preview::extents<int>>);
  static_assert(
      !std::is_constructible_v<layout_wrapping_integral<4>::mapping<preview::extents<int>>, const preview::extents<int>&>);
...
constexpr bool test() {
  ...
  // sanity check
  static_assert(std::is_constructible_v<mds_t, float*, preview::extents<int, 3, D, D>>);
  // wrong size
  static_assert(!std::is_constructible_v<mds_t, float*, preview::extents<int, D, D>>);
  static_assert(!std::is_constructible_v<mds_t, float*, preview::extents<int, D, D, D, D>>);
  // wrong type in general: note the extents constructor does NOT convert, since it takes by const&
  static_assert(!std::is_constructible_v<mds_t, float*, preview::extents<int, D, D, D>>);
  static_assert(!std::is_constructible_v<mds_t, float*, preview::extents<unsigned, 3, D, D>>);

  // test non-constructibility from wrong handle_type
  static_assert(!std::is_constructible_v<mds_t, const float*, preview::extents<int, 3, D, D>>);
