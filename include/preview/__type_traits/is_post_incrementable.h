...
#ifndef PREVIEW_TYPE_TRAITS_IS_POST_INCREMENTABLE_H_
#define PREVIEW_TYPE_TRAITS_IS_POST_INCREMENTABLE_H_
...
struct is_post_incrementable<T, Constraint, CallArgs, Proj, void_t<decltype(std::declval<T>()++)>>
    : meta::bind_first<Constraint, Proj<decltype(std::declval<T>()++)>, CallArgs> {};

} // namespace preview

#endif // PREVIEW_TYPE_TRAITS_IS_POST_INCREMENTABLE_H_
