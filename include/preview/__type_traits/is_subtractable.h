...
#ifndef PREVIEW_TYPE_TRAITS_IS_SUBTRACTABLE_H_
#define PREVIEW_TYPE_TRAITS_IS_SUBTRACTABLE_H_
...
struct is_subtractable<T, U, Constraint, CallArgs, Proj, void_t<decltype(std::declval<T>() - std::declval<U>())>>
    : meta::bind_first<Constraint, Proj<decltype(std::declval<T>() - std::declval<U>())>, CallArgs> {};

} // namespace preview

#endif // PREVIEW_TYPE_TRAITS_IS_SUBTRACTABLE_H_
