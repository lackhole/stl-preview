...
//--------------------------------------------------------------------------------------//
  ...
  //
  //  template<class T> inline BOOST_CONSTEXPR T endian_reverse( T x ) noexcept;

  //  reverse byte order unless native endianness is big
  template <class EndianReversible >
    inline BOOST_CONSTEXPR EndianReversible big_to_native(EndianReversible x) noexcept;
    //  Returns: x if native endian order is big, otherwise endian_reverse(x)
  template <class EndianReversible >
    inline BOOST_CONSTEXPR EndianReversible native_to_big(EndianReversible x) noexcept;
    //  Returns: x if native endian order is big, otherwise endian_reverse(x)

  //  reverse byte order unless native endianness is little
  template <class EndianReversible >
    inline BOOST_CONSTEXPR EndianReversible little_to_native(EndianReversible x) noexcept;
    //  Returns: x if native endian order is little, otherwise endian_reverse(x)
  template <class EndianReversible >
    inline BOOST_CONSTEXPR EndianReversible native_to_little(EndianReversible x) noexcept;
    //  Returns: x if native endian order is little, otherwise endian_reverse(x)

  //  generic conditional reverse byte order
  template <order From, order To,
    class EndianReversible>
      inline BOOST_CONSTEXPR EndianReversible conditional_reverse(EndianReversible from) noexcept;
...
template<class EndianReversible>
inline BOOST_CONSTEXPR EndianReversible conditional_reverse_impl( EndianReversible x, std::true_type ) noexcept
{
    return x;
}

template<class EndianReversible>
inline BOOST_CONSTEXPR EndianReversible conditional_reverse_impl( EndianReversible x, std::false_type ) noexcept
{
...
inline BOOST_CONSTEXPR EndianReversible conditional_reverse( EndianReversible x ) noexcept
{
...
inline std::int16_t load_little_s16( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int16_t, 2, order::little>( p );
}

inline std::uint16_t load_little_u16( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint16_t, 2, order::little>( p );
}

inline std::int16_t load_big_s16( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int16_t, 2, order::big>( p );
}

inline std::uint16_t load_big_u16( unsigned char const * p ) noexcept
{
...
inline std::int32_t load_little_s24( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int32_t, 3, order::little>( p );
}

inline std::uint32_t load_little_u24( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint32_t, 3, order::little>( p );
}

inline std::int32_t load_big_s24( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int32_t, 3, order::big>( p );
}

inline std::uint32_t load_big_u24( unsigned char const * p ) noexcept
{
...
inline std::int32_t load_little_s32( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int32_t, 4, order::little>( p );
}

inline std::uint32_t load_little_u32( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint32_t, 4, order::little>( p );
}

inline std::int32_t load_big_s32( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int32_t, 4, order::big>( p );
}

inline std::uint32_t load_big_u32( unsigned char const * p ) noexcept
{
...
inline std::int64_t load_little_s40( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 5, order::little>( p );
}

inline std::uint64_t load_little_u40( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint64_t, 5, order::little>( p );
}

inline std::int64_t load_big_s40( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 5, order::big>( p );
}

inline std::uint64_t load_big_u40( unsigned char const * p ) noexcept
{
...
inline std::int64_t load_little_s48( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 6, order::little>( p );
}

inline std::uint64_t load_little_u48( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint64_t, 6, order::little>( p );
}

inline std::int64_t load_big_s48( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 6, order::big>( p );
}

inline std::uint64_t load_big_u48( unsigned char const * p ) noexcept
{
...
inline std::int64_t load_little_s56( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 7, order::little>( p );
}

inline std::uint64_t load_little_u56( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint64_t, 7, order::little>( p );
}

inline std::int64_t load_big_s56( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 7, order::big>( p );
}

inline std::uint64_t load_big_u56( unsigned char const * p ) noexcept
{
...
inline std::int64_t load_little_s64( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 8, order::little>( p );
}

inline std::uint64_t load_little_u64( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::uint64_t, 8, order::little>( p );
}

inline std::int64_t load_big_s64( unsigned char const * p ) noexcept
{
    return boost::endian::endian_load<std::int64_t, 8, order::big>( p );
}

inline std::uint64_t load_big_u64( unsigned char const * p ) noexcept
{
