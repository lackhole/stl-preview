...
struct core_arch_operations_msvc_x86_base
{
    ...
    static BOOST_FORCEINLINE void fence_before(std::memory_order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    }

    static BOOST_FORCEINLINE void fence_after(std::memory_order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    }

    static BOOST_FORCEINLINE void fence_after_load(std::memory_order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    ...
    public core_arch_operations_msvc_x86_base
{
    ...
    static BOOST_FORCEINLINE void store(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
    ...
    static BOOST_FORCEINLINE storage_type load(storage_type const volatile& storage, std::memory_order order) noexcept
    {
    ...
    static BOOST_FORCEINLINE storage_type fetch_sub(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        typedef typename boost::std::make_signed< storage_type >::type signed_storage_type;
        return Derived::fetch_add(storage, static_cast< storage_type >(-static_cast< signed_storage_type >(v)), order);
    }

    static BOOST_FORCEINLINE bool compare_exchange_weak(
        storage_type volatile& storage, storage_type& expected, storage_type desired, std::memory_order success_order, std::memory_order failure_order) noexcept
    {
        return Derived::compare_exchange_strong(storage, expected, desired, success_order, failure_order);
    }

    static BOOST_FORCEINLINE bool test_and_set(storage_type volatile& storage, std::memory_order order) noexcept
    {
        return !!Derived::exchange(storage, (storage_type)1, order);
    }

    static BOOST_FORCEINLINE void clear(storage_type volatile& storage, std::memory_order order) noexcept
    {
    ...
    public core_arch_operations_msvc_x86< 4u, Signed, Interprocess, core_arch_operations< 4u, Signed, Interprocess > >
{
    ...
    static BOOST_FORCEINLINE storage_type fetch_add(storage_type volatile& storage, storage_type v, std::memory_order) noexcept
    {
        return static_cast< storage_type >(BOOST_ATOMIC_INTERLOCKED_EXCHANGE_ADD(&storage, v));
    }

    static BOOST_FORCEINLINE storage_type exchange(storage_type volatile& storage, storage_type v, std::memory_order) noexcept
    {
        return static_cast< storage_type >(BOOST_ATOMIC_INTERLOCKED_EXCHANGE(&storage, v));
    }

    static BOOST_FORCEINLINE bool compare_exchange_strong(
        storage_type volatile& storage, storage_type& expected, storage_type desired, std::memory_order, std::memory_order) noexcept
    {
    ...
    static BOOST_FORCEINLINE storage_type fetch_and(storage_type volatile& storage, storage_type v, std::memory_order) noexcept
    {
        return static_cast< storage_type >(BOOST_ATOMIC_INTERLOCKED_AND(&storage, v));
    }
#else
    static BOOST_FORCEINLINE storage_type fetch_and(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        storage_type res = storage;
        while (!compare_exchange_strong(storage, res, res & v, order, std::memory_order_relaxed)) {}
        return res;
    }
#endif

#if defined(BOOST_ATOMIC_INTERLOCKED_OR)
    static BOOST_FORCEINLINE storage_type fetch_or(storage_type volatile& storage, storage_type v, std::memory_order) noexcept
    {
        return static_cast< storage_type >(BOOST_ATOMIC_INTERLOCKED_OR(&storage, v));
    }
#else
    static BOOST_FORCEINLINE storage_type fetch_or(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        storage_type res = storage;
        while (!compare_exchange_strong(storage, res, res | v, order, std::memory_order_relaxed)) {}
        return res;
    ...
    static BOOST_FORCEINLINE storage_type fetch_xor(storage_type volatile& storage, storage_type v, std::memory_order) noexcept
    {
        return static_cast< storage_type >(BOOST_ATOMIC_INTERLOCKED_XOR(&storage, v));
    }
#else
    static BOOST_FORCEINLINE storage_type fetch_xor(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        storage_type res = storage;
        while (!compare_exchange_strong(storage, res, res ^ v, order, std::memory_order_relaxed)) {}
        return res;
    ...
    public core_arch_operations_msvc_x86< 1u, Signed, Interprocess, core_arch_operations< 1u, Signed, Interprocess > >
{
    ...
    static BOOST_FORCEINLINE storage_type fetch_add(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
    ...
    static BOOST_FORCEINLINE storage_type exchange(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        base_type::fence_before(order);
        __asm
        {
    ...
    static BOOST_FORCEINLINE bool compare_exchange_strong(
        storage_type volatile& storage, storage_type& expected, storage_type desired, std::memory_order success_order, std::memory_order) noexcept
    {
...
struct msvc_dcas_x86
{
    ...
    static BOOST_FORCEINLINE void store(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        ...
        if (BOOST_LIKELY(order != std::memory_order_seq_cst && ((uintptr_t)p & 7u) == 0u))
        {
    ...
    static BOOST_FORCEINLINE storage_type load(storage_type const volatile& storage, std::memory_order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
...
struct msvc_dcas_x86_64
{
    ...
    {
#if defined(__AVX__)
        if (BOOST_LIKELY(order != std::memory_order_seq_cst && (((uintptr_t)&storage) & 15u) == 0u))
        {
            BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
            __m128i value;
