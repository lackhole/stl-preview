...
protected:
    typedef std::remove_cv_t< value_type > unqualified_value_type;
    typedef std::conditional_t<
        ...
        atomics::detail::core_operations_emulated< sizeof(value_type), std::alignment_of< value_type >::value, Signed, Interprocess >
    > core_operations;
...
public:
    static BOOST_CONSTEXPR_OR_CONST std::size_t required_alignment = std::alignment_of< value_type >::value <= core_operations::storage_alignment ? core_operations::storage_alignment : std::alignment_of< value_type >::value;
...
public:
    BOOST_FORCEINLINE explicit base_atomic_ref_common(value_type& v) noexcept : m_value(std::addressof(v))
    {
...
#else
    typedef std::integral_constant< bool, std::alignment_of< value_type >::value <= core_operations::storage_alignment > cxchg_use_bitwise_cast;
#endif
...
public:
    ...
    BOOST_FORCEINLINE void store(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
    ...
    BOOST_FORCEINLINE value_type load(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
    ...
    BOOST_FORCEINLINE value_type exchange(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
    ...
    BOOST_FORCEINLINE bool compare_exchange_strong(value_type& expected, value_arg_type desired, std::memory_order success_order, std::memory_order failure_order) const noexcept
    {
    ...
    BOOST_FORCEINLINE bool compare_exchange_strong(value_type& expected, value_arg_type desired, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return compare_exchange_strong(expected, desired, order, atomics::detail::deduce_failure_order(order));
    }

    BOOST_FORCEINLINE bool compare_exchange_weak(value_type& expected, value_arg_type desired, std::memory_order success_order, std::memory_order failure_order) const noexcept
    {
        assert(failure_order != std::memory_order_release);
        assert(failure_order != std::memory_order_acq_rel);
        assert(cas_failure_order_must_not_be_stronger_than_success_order(success_order, failure_order));
    ...
    BOOST_FORCEINLINE bool compare_exchange_weak(value_type& expected, value_arg_type desired, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return compare_exchange_weak(expected, desired, order, atomics::detail::deduce_failure_order(order));
    }

    BOOST_FORCEINLINE value_type wait(value_arg_type old_val, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        assert(order != std::memory_order_release);
        assert(order != std::memory_order_acq_rel);
...
private:
    BOOST_FORCEINLINE bool compare_exchange_strong_impl(value_type& expected, value_arg_type desired, std::memory_order success_order, std::memory_order failure_order, std::false_type) const noexcept
    {
    ...
    BOOST_FORCEINLINE bool compare_exchange_strong_impl(value_type& expected, value_arg_type desired, std::memory_order success_order, std::memory_order failure_order, std::true_type) const noexcept
    {
    ...
    BOOST_FORCEINLINE bool compare_exchange_weak_impl(value_type& expected, value_arg_type desired, std::memory_order success_order, std::memory_order failure_order, std::false_type) const noexcept
    {
    ...
    BOOST_FORCEINLINE bool compare_exchange_weak_impl(value_type& expected, value_arg_type desired, std::memory_order success_order, std::memory_order failure_order, std::true_type) const noexcept
    {
        storage_type old_value = atomics::detail::bitwise_cast< storage_type >(expected);
...
class base_atomic_ref< T, int, Interprocess > :
    public base_atomic_ref_common< T, std::is_signed< T >::value, Interprocess >
{
private:
    typedef base_atomic_ref_common< T, std::is_signed< T >::value, Interprocess > base_type;
...
public:
    ...
    BOOST_FORCEINLINE value_type fetch_add(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(core_operations::fetch_add(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type fetch_sub(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
    ...
    BOOST_FORCEINLINE value_type fetch_and(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(core_operations::fetch_and(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type fetch_or(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(core_operations::fetch_or(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type fetch_xor(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
    ...
    BOOST_FORCEINLINE value_type fetch_negate(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::fetch_negate(this->storage(), order));
    }

    BOOST_FORCEINLINE value_type fetch_complement(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::fetch_complement(this->storage(), order));
    }

    BOOST_FORCEINLINE value_type add(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::add(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type sub(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::sub(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type negate(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::negate(this->storage(), order));
    }

    BOOST_FORCEINLINE value_type bitwise_and(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::bitwise_and(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type bitwise_or(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::bitwise_or(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type bitwise_xor(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::bitwise_xor(this->storage(), static_cast< storage_type >(v), order));
    }

    BOOST_FORCEINLINE value_type bitwise_complement(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(extra_operations::bitwise_complement(this->storage(), order));
    }

    BOOST_FORCEINLINE void opaque_add(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_add(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE void opaque_sub(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_sub(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE void opaque_negate(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_negate(this->storage(), order);
    }

    BOOST_FORCEINLINE void opaque_and(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_and(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE void opaque_or(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_or(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE void opaque_xor(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_xor(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE void opaque_complement(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        extra_operations::opaque_complement(this->storage(), order);
    }

    BOOST_FORCEINLINE bool add_and_test(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::add_and_test(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE bool sub_and_test(difference_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::sub_and_test(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE bool negate_and_test(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::negate_and_test(this->storage(), order);
    }

    BOOST_FORCEINLINE bool and_and_test(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::and_and_test(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE bool or_and_test(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::or_and_test(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE bool xor_and_test(value_arg_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::xor_and_test(this->storage(), static_cast< storage_type >(v), order);
    }

    BOOST_FORCEINLINE bool complement_and_test(std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return extra_operations::complement_and_test(this->storage(), order);
    }

    BOOST_FORCEINLINE bool bit_test_and_set(unsigned int bit_number, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        assert(bit_number < sizeof(value_type) * 8u);
        return extra_operations::bit_test_and_set(this->storage(), bit_number, order);
    }

    BOOST_FORCEINLINE bool bit_test_and_reset(unsigned int bit_number, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        assert(bit_number < sizeof(value_type) * 8u);
        return extra_operations::bit_test_and_reset(this->storage(), bit_number, order);
    }

    BOOST_FORCEINLINE bool bit_test_and_complement(unsigned int bit_number, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        assert(bit_number < sizeof(value_type) * 8u);
...
private:
#if defined(BOOST_ATOMIC_DETAIL_STORAGE_TYPE_MAY_ALIAS) || defined(BOOST_ATOMIC_NO_CLEAR_PADDING)
    typedef std::integral_constant< bool, atomics::detail::value_size_of< value_type >::value != sizeof(storage_type) > has_padding_bits;
#endif
#if !defined(BOOST_ATOMIC_DETAIL_STORAGE_TYPE_MAY_ALIAS) || !defined(BOOST_ATOMIC_NO_CLEAR_PADDING)
    typedef std::true_type cxchg_use_bitwise_cast;
#else
    typedef std::integral_constant< bool, has_padding_bits::value || std::alignment_of< value_type >::value <= core_operations::storage_alignment > cxchg_use_bitwise_cast;
#endif
...
public:
    ...
    BOOST_FORCEINLINE value_type exchange(value_type v, std::memory_order order = std::memory_order_seq_cst) const noexcept
    {
        return atomics::detail::bitwise_cast< value_type >(core_operations::exchange(this->storage(), atomics::detail::bitwise_cast< storage_type >(v), order));
    }
