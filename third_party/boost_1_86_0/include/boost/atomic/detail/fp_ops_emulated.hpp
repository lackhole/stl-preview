    ...
    public Base
{
    ...
    static value_type fetch_add(storage_type volatile& storage, value_type v, std::memory_order) noexcept
    {
    ...
    static value_type fetch_sub(storage_type volatile& storage, value_type v, std::memory_order) noexcept
    {
        static_assert(!base_type::is_interprocess, "Boost.Atomic: operation invoked on a non-lock-free inter-process atomic object");
        storage_type& s = const_cast< storage_type& >(storage);
        scoped_lock lock(&storage);
