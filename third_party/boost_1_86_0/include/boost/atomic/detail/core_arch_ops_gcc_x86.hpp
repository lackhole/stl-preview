...
struct core_arch_operations_gcc_x86_base
{
    ...
    static BOOST_FORCEINLINE void fence_before(std::memory_order order) noexcept
    {
        if ((static_cast< unsigned int >(order) & static_cast< unsigned int >(std::memory_order_release)) != 0u)
            __asm__ __volatile__ ("" ::: "memory");
    }

    static BOOST_FORCEINLINE void fence_after(std::memory_order order) noexcept
    {
...
struct gcc_dcas_x86
{
    ...
    static BOOST_FORCEINLINE void store(storage_type volatile& storage, storage_type v, std::memory_order order) noexcept
    {
        if (BOOST_LIKELY(order != std::memory_order_seq_cst && (((uintptr_t)&storage) & 7u) == 0u))
        {
#if defined(__SSE__)
#if defined(__SSE2__)
