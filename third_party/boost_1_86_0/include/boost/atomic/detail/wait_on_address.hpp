...
BOOST_ATOMIC_DECL void initialize_wait_functions() noexcept;

BOOST_FORCEINLINE void ensure_wait_functions_initialized() noexcept
{
    ...
    if (BOOST_LIKELY(once_flag_operations::load(wait_functions_once_flag.m_flag, boost::std::memory_order_acquire) == 0u))
        return;
