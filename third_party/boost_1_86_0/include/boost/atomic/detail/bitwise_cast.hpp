...
{
    BOOST_ATOMIC_DETAIL_MEMSET(reinterpret_cast< unsigned char* >(std::addressof(to)) + ValueSize, 0, sizeof(To) - ValueSize);
}
...
{
    typedef typename std::remove_cv< To >::type unqualified_to_t;
    unqualified_to_t to;
#if !defined(BOOST_ATOMIC_NO_CLEAR_PADDING)
    From from2(from);
    BOOST_ATOMIC_DETAIL_CLEAR_PADDING(std::addressof(from2));
