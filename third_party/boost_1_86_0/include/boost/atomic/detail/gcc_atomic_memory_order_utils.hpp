...
/*!
 * The function converts \c boost::std::memory_order values to the compiler-specific constants.
 *
 ...
 */
BOOST_FORCEINLINE BOOST_CONSTEXPR int convert_memory_order_to_gcc(std::memory_order order) noexcept
{
    return (order == std::memory_order_relaxed ? __ATOMIC_RELAXED : (order == std::memory_order_consume ? __ATOMIC_CONSUME :
        (order == std::memory_order_acquire ? __ATOMIC_ACQUIRE : (order == std::memory_order_release ? __ATOMIC_RELEASE :
        (order == std::memory_order_acq_rel ? __ATOMIC_ACQ_REL : __ATOMIC_SEQ_CST)))));
}
