...
// A note about std::memory_order_consume. Technically, this architecture allows to avoid
// unnecessary memory barrier after consume load since it supports data dependency ordering.
// However, some compiler optimizations may break a seemingly valid code relying on data
// dependency tracking by injecting bogus branches to aid out of order execution.
// This may happen not only in Boost.Atomic code but also in user's code, which we have no
// control of. See this thread: http://lists.boost.org/Archives/boost/2014/06/213890.php.
// For this reason we promote std::memory_order_consume to std::memory_order_acquire.
...
struct core_arch_operations_msvc_arm_base
{
    ...
    static BOOST_FORCEINLINE void fence_before_store(std::memory_order order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();

        if ((static_cast< unsigned int >(order) & static_cast< unsigned int >(std::memory_order_release)) != 0u)
            fence_arch_operations::hardware_full_fence();

        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    }

    static BOOST_FORCEINLINE void fence_after_store(std::memory_order order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    ...
    static BOOST_FORCEINLINE void fence_after_load(std::memory_order order) noexcept
    {
        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();

        if ((static_cast< unsigned int >(order) & (static_cast< unsigned int >(std::memory_order_consume) | static_cast< unsigned int >(std::memory_order_acquire))) != 0u)
            fence_arch_operations::hardware_full_fence();

        BOOST_ATOMIC_DETAIL_COMPILER_BARRIER();
    }

    static BOOST_FORCEINLINE BOOST_CONSTEXPR std::memory_order cas_common_order(std::memory_order success_order, std::memory_order failure_order) noexcept
    {
        // Combine order flags together and promote std::memory_order_consume to std::memory_order_acquire
        return static_cast< std::memory_order >(((static_cast< unsigned int >(failure_order) | static_cast< unsigned int >(success_order)) & ~static_cast< unsigned int >(std::memory_order_consume))
            | (((static_cast< unsigned int >(failure_order) | static_cast< unsigned int >(success_order)) & static_cast< unsigned int >(std::memory_order_consume)) << 1u));
    }
};
