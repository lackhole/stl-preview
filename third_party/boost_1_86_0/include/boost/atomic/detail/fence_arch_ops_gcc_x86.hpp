...
{
    static BOOST_FORCEINLINE void thread_fence(std::memory_order order) noexcept
    {
        ...
        else if ((static_cast< unsigned int >(order) & (static_cast< unsigned int >(std::memory_order_acquire) | static_cast< unsigned int >(std::memory_order_release))) != 0u)
        {
            __asm__ __volatile__ ("" ::: "memory");
        }
    }

    static BOOST_FORCEINLINE void signal_fence(std::memory_order order) noexcept
    {
