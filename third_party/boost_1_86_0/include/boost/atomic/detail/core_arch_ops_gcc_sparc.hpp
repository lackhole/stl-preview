...
struct gcc_sparc_cas_base
{
    ...
    static BOOST_FORCEINLINE void fence_before(std::memory_order order) noexcept
    {
        ...
        else if ((static_cast< unsigned int >(order) & static_cast< unsigned int >(std::memory_order_release)) != 0u)
    ...
    static BOOST_FORCEINLINE void fence_after(std::memory_order order) noexcept
    {
        if (order == std::memory_order_seq_cst)
            __asm__ __volatile__ ("membar #Sync" ::: "memory");
        else if ((static_cast< unsigned int >(order) & (static_cast< unsigned int >(std::memory_order_consume) | static_cast< unsigned int >(std::memory_order_acquire))) != 0u)
            __asm__ __volatile__ ("membar #StoreStore | #LoadStore" ::: "memory");
    }
